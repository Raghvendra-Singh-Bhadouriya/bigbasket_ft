...
function BeveragesTea(){
    const [ beveragesTeaData, setBeveragesTeaData ] = useState([])

    useEffect(() => {
        fetchBeveragesTeaData()
    }, [])

    async function fetchBeveragesTeaData(){
        try {
            const res = await axios({
                method: "get",
                url: `https://bigbasket-bc.onrender.com/beverages/tea`
            })
            const {data} = res.data
            console.log(data)
            setBeveragesTeaData(data)
        } catch (error) {
        ...
        <Box bgColor={"rgb(241, 239, 239)"} pt={5}>
            <Box w={["100%", "90%", "90%", "70%"]} m={"auto"} bg={"transparent"}>
                <Flex to="/" border={"0px solid black"} w={[ "60%", "50%", "40%", "30%"]} justifyContent={"space-between"} bg={"transparent"}>
                    ...
                    <Text fontSize={"small"} color={"gray"} fontWeight={"bold"} bg={"transparent"}>Beverages Tea</Text>
                </Flex>
                <Text mt={5} fontSize={"large"} color={"gray"}  bg={"transparent"}>Beverages Tea</Text>

import { Box, Card, Image, Text, Heading, Flex, Grid, GridItem, Button } from "@chakra-ui/react"
...
function FruitsAndVeggies(){
    const [ fruitVeggiesData, setFruitsVeggiesData ] = useState([])

    useEffect(() => {
        fetchFruitsVeggiesData()
    }, [])

    async function fetchFruitsVeggiesData(){
        try {
            const res = await axios({
                method: "get",
                url: `https://bigbasket-bc.onrender.com/fruits-veggies`
            })
            const {data} = res.data
            console.log(data)
            setFruitsVeggiesData(data)
        } catch (error) {
        ...
        <Box bgColor={"rgb(241, 239, 239)"} pt={5}>
            <Box w={["100%", "90%", "90%", "70%"]} m={"auto"} bg={"transparent"}>
                <Flex to="/" border={"0px solid black"} w={[ "60%", "50%", "40%", "30%"]} justifyContent={"space-between"} bg={"transparent"}>
                    ...
                    <Text fontSize={"small"} color={"gray"} fontWeight={"bold"} bg={"transparent"}>Exotic Fruits & Veggies</Text>
                </Flex>
                <Text mt={5} fontSize={"large"} color={"gray"}  bg={"transparent"}>Exotic Fruits & Veggies</Text>
